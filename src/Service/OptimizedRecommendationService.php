...
/**
 * Service de recommandation d'utilisateurs optimisé
 *
 * Ce service calcule des recommandations personnalisées d'utilisateurs basées sur :
 * - Analyse comportementale (60%) : likes, activité, popularité
 * - Spécialisation académique (16%)
 * - Thématique de recherche (12%)
 * - Lieu d'affiliation (8%)
 * - Tags de profil (4%)
 *
 * Fonctionnalités :
 * - Calcul de scores de similarité entre utilisateurs
 * - Application de boost de diversité
 * - Mise en cache des résultats (TTL 1 heure)
...
class OptimizedRecommendationService
{
     ...
     *
     * @param User|null $currentUser L'utilisateur courant (optionnel)
     * @param int $limit Nombre maximum de recommandations à retourner
     * @return array Tableau des utilisateurs recommandés avec leurs scores
     */
    public function calculateRecommendationScores(?User $currentUser = null, int $limit = self::MAX_RECOMMENDATIONS): array
    {
        ...
        $executionTime = round((microtime(true) - $startTime) * 1000, 2, 1);
     ...
     *
     * @param User $currentUser L'utilisateur courant
     * @param User $otherUser L'utilisateur à comparer
     * @param array $behavioralData Données comportementales pré-calculées
     * @param array $categoryScores Référence pour suivre les scores par catégorie
     * @return float Score total entre 0 et 1
     */
     ...
     * @param array $behavioralData Données comportementales pré-calculées
     * @param array $categoryScores Référence pour suivre les scores par catégorie
     * @return float Score comportemental entre 0 et 1
     */
     ...
     * @param User $currentUser L'utilisateur courant
     * @param User $otherUser L'utilisateur à comparer
     * @return float Score traditionnel entre 0 et 1
     */
     ...
     *
     * @param User $user L'utilisateur dont on extrait les données
     ...
     *
     * @param string $location1 Première localisation
     * @param string $location2 Deuxième localisation
     * @return float Similarité entre 0 et 1
     */
     ...
     *
     * @param array $questions1 Réponses de l'utilisateur 1
     * @param array $questions2 Réponses de l'utilisateur 2
     * @return float Similarité entre 0 et 1
     */
     ...
     * @param User $otherUser L'utilisateur à comparer
     * @param array $behavioralData Données comportementales pré-calculées
     * @return array Détails des scores par composante
     */
     ...
     *
     * @param array $scores Tableau des scores par utilisateur
     * @param User $currentUser L'utilisateur courant
     * @return array Tableau des scores mis à jour
     */
     ...
     *
     * @param array $scores Tableau des scores par utilisateur
     * @param array $categoryScores Scores agrégés par catégorie
     * @return array Tableau des scores mis à jour
     */
     ...
     * @param array $categoryScores Scores agrégés par catégorie
     * @return array Catégories principales avec leurs scores
     */
     ...
     *
     * @param User $user L'utilisateur dont on veut vider le cache
     * @return void
     ...
     *
     * @param User $currentUser L'utilisateur courant
     * @param array $behavioralData Données comportementales
     * @return void
     */
    private function debugBehavioralData(User $currentUser, array $behavioralData): void
    {
        ...
        if (empty($userInterests)) {
            ...
            $this->logger->info('  Total likes: {totalLikes}', ['totalLikes' => $totalLikes]);
            foreach ($userInterests as $interest) {
                $percentage = round(($interest['likeCount'] / $totalLikes) * 100, 1, 1);
